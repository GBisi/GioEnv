            ...
            "thing":{
                ...
                "handlers":{
                    "actions":{
                        ...
                        "mediateRooms":"let mediation = { 'temperature': 'medium', 'light': 'low' }; thing.readProperty('rooms').then((rooms) => { if (Object.keys(rooms).length == 0) { console.debug('*** MEDIATE ***: no rooms'); resolve(mediation); return; }; let pref = { 'temperature': [], 'light': [] }; for (const r in rooms) { pref['temperature'].push(rooms[r]['temperature']); pref['light'].push(rooms[r]['light']); }; console.debug('*** MEDIATE ***: total preferences ' + JSON.stringify(pref)); console.debug('*** MEDIATE ***: mediating'); fetch(eaas + 'mediate/avg', { 'method': 'POST', 'headers': { 'Accept': 'application/json', 'Content-Type': 'application/json' }, 'body': JSON.stringify({ 'rounding': 'floor', 'data': pref['temperature'], 'values': ['very_low', 'low', 'medium', 'high', 'very_high'] }) }).then((response) => { return response.json(); }).then((data) => { console.debug('*** MEDIATE ***: temp ' + JSON.stringify(data)); mediation['temperature'] = data['avg'] }).catch((e) => { console.debug(e); }).then(()=>{ fetch(eaas + 'mediate/avg', { 'method': 'POST', 'headers': { 'Accept': 'application/json', 'Content-Type': 'application/json' }, 'body': JSON.stringify({ 'data': pref['light'], 'values': ['low', 'medium', 'high'], 'rounding': 'floor' }) }).then((response) => { return response.json(); }).then((data) => { console.debug('*** MEDIATE ***: light ' + JSON.stringify(data)); mediation['light'] = data['avg']; console.debug('*** MEDIATE ***: result ' + JSON.stringify(mediation)); resolve(mediation); thing.invokeAction('deploy',mediation); }); }).catch((e)=>{console.debug(e); resolve(mediation); thing.invokeAction('deploy',mediation); }) });",
                        "deploy":"console.debug('*** DEPLOY *** start'); console.debug('*** DEPLOY *** getting admin rules'); fetch(s2m+'Admin/rules').then(function(response) { return response.json() }).then(function(data) { let rules = JSON.stringify({ 'rules': data['data'] }); console.debug('*** DEPLOY *** parsing admin rules'); return fetch(eaas + 'parse/rulestolist', { 'method': 'POST', 'headers': { 'Accept': 'application/json', 'Content-Type': 'application/json' }, 'body': rules }) }).then(function(response) { return response.json(); }).then(function(data) {  let facts = '[' + jsontolist(input).join().toLowerCase() + ']'; console.debug('*** DEPLOY *** state:'+facts); fetch(eaas + 'infer', { 'method': 'POST', 'headers': { 'Accept': 'application/json', 'Content-Type': 'application/json' }, 'body': JSON.stringify({ 'rules': data['rules'], 'facts': facts }) }).then((response) => { return response.json(); }).then((data) => { console.debug('*** DEPLOY ***:'+data['actions']); resolve(listtojson(data['actions']));}); }).catch(function(error) { console.debug(error); resolve('Unable to deploy'); }); ",
                        },
                }
            }
