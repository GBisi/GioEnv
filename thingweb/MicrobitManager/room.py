        ...
        self.room = {
            ...
            "handlers":{
                "actions":{
                    ...
                    "mediateUsers":"let user_setup = { 'temperature': [], 'light': [] };let mediation = { 'temperature': 'medium', 'light': 'medium' }; let light = 0; let temp = 0; let arr = []; thing.readAllProperties().then((props) => {delete props['last_indoor_update']; delete props['last_outdoor_update']; props['users']=Object.keys(props['users']).length; props['light'] = props['lightL'].toLowerCase(); props['temperature'] = props['temperatureL'].toLowerCase(); light = props['light']; temp = props['temperature']; let facts = '[' + jsontolist(props).join().toLowerCase() + ']'; console.debug('Mediate:'+facts);thing.readProperty('users').then((users) => { users_len = props['users']; if (users_len == 0) {console.debug('Mediate: no user'); resolve(mediation); return; } for (const user in users) { console.debug('Mediate: preferences of '+user+' getting');fetch(eaas + 'infer', { 'method': 'POST', 'headers': { 'Accept': 'application/json', 'Content-Type': 'application/json' }, 'body': JSON.stringify({ 'rules': users[user]['rules'], 'facts': facts }) }).then((response) => { return response.json(); }).then((data) => {console.debug('Mediate: prefernces of '+user+' -> '+actions); actions = data['actions']; actions = listtojson(actions); if (!('light' in actions)) { actions['light'] = light } if (!('temperature' in actions)) { actions['temperature'] = temp }; user_setup['temperature'].push(actions['temperature']); user_setup['light'].push(actions['light']); return user_setup }).catch((e) => { console.debug(e); users_len--; return user_setup; }).then((data) => { if (data['temperature'].length >= users_len) {console.debug('Mediate: total preferences '+user_setup); return data; } }).then((data)=>{console.log(user_setup); console.debug('Mediate: mediating');fetch(eaas + 'mediate/avg', { 'method': 'POST', 'headers': { 'Accept': 'application/json', 'Content-Type': 'application/json' }, 'body': JSON.stringify({ 'rounding':'floor', 'data':user_setup['temperature'], 'values':['very_low','low','medium','high','very_high'] }) }).then((response) => { return response.json(); }).then((data) => { console.debug(data);mediation['temperature']=data['avg'] }) .catch((e) => { console.debug(e); }) .then( fetch(eaas + 'mediate/avg', { 'method': 'POST', 'headers': { 'Accept': 'application/json', 'Content-Type': 'application/json' }, 'body': JSON.stringify({ 'data':user_setup['light'], 'values':['low','medium','high'], 'rounding':'floor' }) }).then((response) => { return response.json(); }).then((data) => { console.debug(data);mediation['light']=data['avg']; resolve(mediation); }) ); }).catch((e) => { console.debug(e); resolve(mediation);}) }; }); });",
                    },
