        ...
        self.room = {
            ...
            "handlers":{
                "actions":{
                    ...
                    "enter":,"thing.readProperty('users').then((users)=>{if(!(users.hasOwnProperty(input))){fetch(s2m+input+'/rules').then((response)=>{return response.json();}).then((data)=>{fetch(eaas,{'body':data}).then((response1) => {return response1.json();}).then((data1)=>{users[input]=data1;})}).catch((e)=>{users[input] = [];});resolve('User registered');}else{resolve('User already in the room');}});"
                ...
                "properties":{
                    ...
                    "outdoor_light":{
                        "read":'fetch(openweathermap_uvi).then((response) => {return response.json();}).then((data) => {fetch(openweathermap_uvi).then((response) => {return response.json();}).then((data) => {thing.writeProperty("last_outdoor_update", (new Date()).toISOString());resolve((data["value"]/11)*255)}).catch((e)=>{});'
                    },
                    "outdoor_temp":{
